                  ...
                  return segs.map((sg, i) => {
                    ...
                    return (
                      <div key={sg.tarefa + i}
                        ...
                        {height > 16 && <span style={{ fontSize: 7, fontWeight: 700, color: '#000', padding: '0 2px', lineHeight: 1.1, pointerEvents: 'none' }}>{sg.tarefa.length > 8 ? sg.tarefa.slice(0, 7) + '…' : sg.tarefa}</span>}
                      </div>
                    )
                  })
                })()}
...
export default function Indicadores({ usuario }) {
  ...
  return (
    <div>
                    ...
                    return (
                      <div key={maquina} onClick={() => setMaquinaSelecionada({ maquina, tarefas })}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                          ...
                          <div style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 700, color: 'var(--yellow)', flex: 1 }}>{maquina.toUpperCase()}</div>
                          <div style={{ textAlign: 'right' }}>
                          ...
                          {dias5.map(dia => {
                            ...
                            return (
                              <div key={key} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                                <div style={{ flex: 1, width: '100%', position: 'relative', background: 'var(--surface2)', borderRadius: '3px 3px 0 0', border: ehHoje ? '1px solid rgba(0,229,255,.3)' : '1px solid var(--border)', overflow: 'hidden' }}>
                                  {segs.map((sg, i) => {
                                    const bottom = acum * escMin
                                    const height = Math.max(sg.tempoNoDia * escMin, 2)
                                    acum += sg.tempoNoDia
                                    return (
                                      <div key={i} style={{ position: 'absolute', bottom, left: 0, right: 0, height, background: tarefaCores[sg.tarefa] || '#888', opacity: 0.9 }} />
                                    )
                                  })}
                                </div>
      ...
      {maquinaSelecionada && (
        <ModalMaquina maquina={maquinaSelecionada.maquina} tarefas={maquinaSelecionada.tarefas} onClose={() => setMaquinaSelecionada(null)} />
      )}
